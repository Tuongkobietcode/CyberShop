...
export default function InventoryPage() {
  ...
  useEffect(() => {
    async function loadLogs() {
      setLoading(true);
      try {
        const response = await getAdminInventoryLogs({ limit: 100 });
  ...
  const reasons = useMemo(() => ["all", ...new Set(logs.map((log) => log.reason))], [logs]);

  const filteredLogs = useMemo(() => {
    return logs.filter((log) => {
      const keyword = `${log.productName} ${log.sku} ${log.referenceCode}`.toLowerCase();
      const matchesSearch = keyword.includes(search.toLowerCase());
      const matchesReason = reason === "all" ? true : log.reason === reason;
  ...
  const stockOutEvents = logs.filter((log) => log.nextStock === 0).length;
  const restoredEvents = logs.filter((log) => log.delta > 0).length;
    ...
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-3">
        <section className={surface}>
          ...
          <p className="mt-5 text-[2.4rem] font-semibold tracking-[-0.06em] text-black">{logs.length.toLocaleString("en-US")}</p>
        ...
        <section className={surface}>
          ...
          <p className="mt-5 text-[2.4rem] font-semibold tracking-[-0.06em] text-black">{stockOutEvents.toLocaleString("en-US")}</p>
        ...
        <section className={surface}>
          ...
          <p className="mt-5 text-[2.4rem] font-semibold tracking-[-0.06em] text-black">{restoredEvents.toLocaleString("en-US")}</p>
      ...
      <section className={surface}>
        ...
        {loading ? (
          <div className="mt-6 rounded-2xl border border-dashed border-black/10 px-6 py-12 text-center text-sm text-black/45">Loading inventory activity...</div>
        ) : (
                ...
                {pagedLogs.length === 0 ? (
                  ...
                  pagedLogs.map((log) => (
                    <tr key={log.id} className="border-t border-black/6">
                      ...
                      <td className="px-5 py-4">
                        <span className={["inline-flex rounded-full px-3 py-1 text-xs font-semibold", log.delta > 0 ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-600"].join(" ")}>
                      ...
                      <td className="px-5 py-4 text-black/45">
                        {log.referenceCode || <span className="inline-flex items-center gap-2"><History className="h-4 w-4" />Manual</span>}
                      </td>
