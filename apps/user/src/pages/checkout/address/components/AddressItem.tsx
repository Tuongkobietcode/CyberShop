  ...
  return (
    ...
    >
      <label className="flex items-start pt-1">
        <input type="radio" checked={checked} onChange={onSelect} className="mt-1 h-5 w-5 accent-[var(--accent)]" />
      </label>

      <div>
        <div className="flex flex-wrap items-center gap-3">
          <h3 className="text-2xl font-semibold tracking-[-0.05em] text-[var(--text-primary)]">{address.fullName}</h3>
          <span className="rounded-full border border-[var(--line-soft)] bg-white/[0.05] px-3 py-1 text-[11px] uppercase tracking-[0.18em] text-[var(--text-secondary)]">
            {address.label}
          </span>
          {address.isDefault ? (
      ...
      <div className="flex items-center gap-3 sm:justify-end">
        <button
          type="button"
          onClick={onEdit}
          className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-[var(--line-soft)] text-[var(--text-secondary)] transition hover:border-[rgba(255,255,255,0.22)] hover:text-[var(--text-primary)]"
        >
