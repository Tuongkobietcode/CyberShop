...
export default function ReviewSection({ reviews }: { reviews: ProductReview[] }) {
  return (
    <section className="space-y-8">
      <h2 className="text-[2rem] font-semibold tracking-[-0.04em] text-white">Reviews</h2>

      <div className="grid gap-8 lg:grid-cols-[220px_minmax(0,1fr)]">
        <div className="cy-panel p-6 text-center">
          <p className="font-mono text-[3rem] font-semibold tracking-[-0.06em] text-white">4.8</p>
          <p className="mt-2 text-sm text-white/35">of 125 reviews</p>
        ...
        <div className="cy-panel space-y-3 p-6 pt-7">
          {[
            ...
            <div key={label as string} className="grid grid-cols-[140px_minmax(0,1fr)_40px] items-center gap-4 text-sm text-white/70">
              <span>{label}</span>
              <div className="h-1 rounded-full bg-white/10"><div className="h-full rounded-full bg-[#ffbf69]" style={{ width: `${width}%` }} /></div>
            ...
            <div className="flex gap-4">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <h3 className="text-[1.05rem] font-semibold text-white">{review.author}</h3>
                    <div className="mt-2"><RatingStars value={review.rating} /></div>
                  </div>
                  <p className="text-sm text-white/30">{review.date}</p>
                </div>
                <p className="mt-4 text-[15px] leading-7 text-white/58">{review.content}</p>
